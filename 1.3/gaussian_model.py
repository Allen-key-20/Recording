...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, ping = 0):
    ...
    def downOpacity(self, op):
        self._opacity -= op

    def s2tos3(self):
        three1 = torch.mul(self._scaling2.min(dim=1).values, self.ping)
        three2 = torch.div(self._scaling2.max(dim=1).values, self.ping)
        three = torch.max(three1, three2)[..., None]
        self._scaling = torch.cat((self._scaling2, three), dim=1)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        if self.ping:
            scales2 = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 2)
    ...
    def training_setup(self, training_args):
        ...
        if self.ping: #如果采用平高斯，则只优化前2个缩放尺度
            l = [
    ...
    def load_ply(self, path):
        ...
        self._mask = nn.Parameter(torch.ones((xyz.shape[0], 1), device='cuda').requires_grad_(True))
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, focal_length, ratio, iteration):
        ...
        if ratio:
            small = (extent/focal_length) * ratio
            small_mask = torch.abs(self.get_scaling.mean(dim=1)) < small
